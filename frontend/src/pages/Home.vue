...
function getOverviewStatKey(label) {
  const map = {
    ...
    'Trainings in 3 days': 'statTrainingsIn3Days',
    'Next training countdown': 'statNextTrainingCountdown',
    'Training hours in 3 days': 'statTrainingHoursIn3Days',
    'My groups': 'statMyGroups',
    'Attendance across my groups': 'statAttendanceAcrossMyGroups',
    'Attendance rate': 'statAttendanceRate'
  }
...
const getOverviewStatPresentation = (statKey) => {
  switch (statKey) {
    case 'statTotalUsers':
      return { icon: 'mdi-account-group-outline', tone: 'users', caption: t('pages.home.statCaptionActiveAccounts') }
    case 'statCoaches':
      return { icon: 'mdi-whistle-outline', tone: 'coaches', caption: t('pages.home.statCaptionCoachingStaff') }
    case 'statGroups':
    case 'statMyGroups':
      return { icon: 'mdi-google-circles-communities', tone: 'groups', caption: t('pages.home.statCaptionGroupsInView') }
    case 'statPendingPayments':
      return { icon: 'mdi-cash-clock', tone: 'payments', caption: t('pages.home.statCaptionNeedFollowUp') }
    case 'statTrainingsIn3Days':
      return { icon: 'mdi-calendar-range-outline', tone: 'time', caption: t('pages.home.statCaptionScheduledSoon') }
    case 'statNextTrainingCountdown':
      return { icon: 'mdi-timer-outline', tone: 'time', caption: t('pages.home.statCaptionUntilNextSession') }
    case 'statTrainingHoursIn3Days':
      return { icon: 'mdi-timetable', tone: 'time', caption: t('pages.home.statCaptionCoachWorkload') }
    case 'statAttendanceAcrossMyGroups':
      return { icon: 'mdi-clipboard-check-outline', tone: 'attendance', caption: t('pages.home.statCaptionMarkedRecords') }
    case 'statAttendanceRate':
      return { icon: 'mdi-chart-donut', tone: 'attendance', caption: t('pages.home.statCaptionPresentVsAbsent') }
    default:
      return { icon: 'mdi-chart-box-outline', tone: 'default', caption: t('pages.home.statCaptionOverviewMetric') }
  }
}
...
const displayedOverviewStats = computed(() =>
  overviewStats.value.map((item) =>
    {
      const statKey = getOverviewStatKey(item.label)

      return {
      ...item,
      ...getOverviewStatPresentation(statKey),
      label: translateOverviewStatLabel(item.label),
      value: statKey === 'statNextTrainingCountdown'
        ? nextTrainingCountdown.value
        : isCoach.value && statKey === 'statTrainingHoursIn3Days'
