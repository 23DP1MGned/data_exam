<template>
  <v-app>
    <v-main class="admin-groups-page">
      ...
      <div class="admin-groups-shell admin-theme-root" :class="{ 'admin-groups-shell-dark': darkMode, 'admin-theme-root-dark': darkMode }">
        <v-navigation-drawer
...
const overviewStats = computed(() => [
  {
    label: t('pages.adminGroups.statTotalGroups'),
    value: groups.value.length,
    caption: t('pages.adminGroups.statCaptionActiveGroups'),
    icon: 'mdi-google-circles-communities',
    tone: 'groups'
  },
  {
    label: t('pages.adminGroups.statAssignedCoaches'),
    value: new Set(groups.value.map((item) => item.trainer).filter(Boolean)).size,
    caption: t('pages.adminGroups.statCaptionAssignedCoaches'),
    icon: 'mdi-whistle-outline',
    tone: 'coaches'
  },
  {
    label: t('pages.adminGroups.statChildrenLinked'),
    value: groups.value.reduce((acc, item) => acc + (item.students || 0), 0),
    caption: t('pages.adminGroups.statCaptionStudentSeats'),
    icon: 'mdi-account-multiple-outline',
    tone: 'users'
  },
  {
    label: t('pages.adminGroups.statAveragePrice'),
    value: formatCurrency(averagePrice.value),
    caption: t('pages.adminGroups.statCaptionAverageMonthlyFee'),
