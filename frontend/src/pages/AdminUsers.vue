          ...
          <div class="mobile-drawer-inner">
            ...
            <nav class="mobile-nav-list">
              <v-btn
                ...
                variant="text"
                class="nav-item"
                :class="{ 'nav-item-active': item.to === '/admin-users' }"
                block
              ...
              <div class="create-fields-grid">
                ...
                <template v-if="form.role === 'parent'">
                  ...
                  <v-text-field
                    ...
                    class="create-field"
                  />
                  <div class="create-field create-field-full linked-children-field">
                    <div class="linked-children-label">{{ t('pages.adminUsers.linkedChildrenSelect') }}</div>
                    <v-text-field
                      v-if="allChildSelectionOptions.length"
                      v-model="form.child_search"
                      :label="t('pages.adminUsers.searchChildren')"
                      variant="outlined"
                      density="comfortable"
                      hide-details
                      prepend-inner-icon="mdi-magnify"
                      class="linked-children-search"
                    />
                    <div v-if="childSelectionOptions.length" class="linked-children-list" :class="{ 'linked-children-list-dark': darkMode }">
                      <label
                        v-for="child in childSelectionOptions"
                        :key="child.id"
                        class="linked-child-option"
                        :class="{ 'linked-child-option-dark': darkMode }"
                      >
                        <input
                          v-model="form.child_ids"
                          :value="child.id"
                          type="checkbox"
                          class="linked-child-checkbox"
                        >
                        <div class="linked-child-copy">
                          <div class="linked-child-name">{{ child.name }}</div>
                          <div class="linked-child-meta">{{ child.email }}</div>
                        </div>
                      </label>
                    </div>
                    <div v-else class="linked-children-empty">
                      {{ t('pages.adminUsers.noChildOptions') }}
                    </div>
                  </div>
                </template>
...
import StatsOverviewGrid from '../components/StatsOverviewGrid.vue'
...
const overviewStats = computed(() => {
  ...
  return [
    {
      label: t('pages.adminUsers.statTotalUsers'),
      value: totals.total,
      caption: t('pages.adminUsers.statCaptionRegisteredProfiles'),
      icon: 'mdi-account-group-outline',
      tone: 'users'
    },
    {
      label: t('pages.adminUsers.statAdmins'),
      value: totals.admin,
      caption: t('pages.adminUsers.statCaptionAdministratorAccounts'),
      icon: 'mdi-shield-account-outline',
      tone: 'coaches'
    },
    {
      label: t('pages.adminUsers.statParents'),
      value: totals.parent,
      caption: t('pages.adminUsers.statCaptionFamilyAccounts'),
      icon: 'mdi-account-heart-outline',
      tone: 'groups'
    },
    {
      label: t('pages.adminUsers.statChildren'),
      value: totals.child,
      caption: t('pages.adminUsers.statCaptionJuniorAthletes'),
      icon: 'mdi-human-child',
      tone: 'attendance'
    },
    {
      label: t('pages.adminUsers.statAdults'),
      value: totals.adult,
      caption: t('pages.adminUsers.statCaptionAdultMembers'),
...
const allChildSelectionOptions = computed(() =>
  users.value
    .filter((item) => item.role === 'child' && item.id !== editingUserId.value)
    .map((item) => ({
      id: item.id,
      name: item.full_name,
      email: item.email
...
const childSelectionOptions = computed(() => {
  const query = form.value.child_search?.trim().toLowerCase() ?? ''

  if (!query) return allChildSelectionOptions.value

  return allChildSelectionOptions.value.filter((item) =>
...
function openEditDialog(item) {
  ...
  form.value = {
    ...
    child_ids: item.role === 'parent' ? (item.children?.map((child) => child.id) ?? []) : [],
    child_search: '',
...
.create-dialog-card-dark .linked-children-label {
  color: #d7e4f8;
}
...
.linked-children-list {
  ...
  overflow: auto;
  border-radius: 18px;
  border: 1px solid rgba(221, 230, 242, 0.96);
  background: rgba(255, 255, 255, 0.82);
}

.linked-children-list-dark {
  border-color: rgba(66, 84, 118, 0.64);
...
.linked-child-option-dark {
  border-color: rgba(63, 80, 114, 0.58);
  background: rgba(17, 25, 40, 0.88);
}

.linked-child-checkbox {
  margin-top: 2px;
  width: 16px;
  height: 16px;
...
.create-dialog-card-dark .linked-child-name {
  color: #f3f7ff;
}

.linked-child-meta,
.linked-children-empty {
  margin-top: 2px;
  font-size: 0.84rem;
  color: #738196;
}

.create-dialog-card-dark .linked-child-meta,
.create-dialog-card-dark .linked-children-empty {
  color: #9db0cd;
}
...
.create-dialog-card-dark :deep(.role-select-field .v-field) {
  background:
    linear-gradient(180deg, rgba(17, 25, 40, 0.94), rgba(20, 29, 46, 0.9));
}

.create-dialog-card-dark :deep(.create-field .v-field:hover) {
...
.create-dialog-card-dark :deep(.role-select-field .v-field__append-inner) {
  color: #a7bbdc;
}
