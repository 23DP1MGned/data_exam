...
function getOverviewStatKey(label) {
  const map = {
    'Total users': 'statTotalUsers',
    Coaches: 'statCoaches',
    Groups: 'statGroups',
    'Pending payments': 'statPendingPayments'
  }

  return map[label] ?? null
}

const getOverviewStatPresentation = (statKey) => {
  switch (statKey) {
    case 'statTotalUsers':
      return {
        icon: 'mdi-account-group-outline',
        tone: 'users',
        caption: t('pages.adminHome.statCaptionActiveAccounts')
      }
    case 'statCoaches':
      return {
        icon: 'mdi-whistle-outline',
        tone: 'coaches',
        caption: t('pages.adminHome.statCaptionAssignedToGroups')
      }
    case 'statGroups':
      return {
        icon: 'mdi-google-circles-communities',
        tone: 'groups',
        caption: t('pages.adminHome.statCaptionTrainingUnits')
      }
    case 'statPendingPayments':
      return {
        icon: 'mdi-cash-clock',
        tone: 'payments',
        caption: t('pages.adminHome.statCaptionNeedAttention')
      }
    default:
      return {
        icon: 'mdi-chart-box-outline',
        tone: 'default',
        caption: t('pages.adminHome.statCaptionSystemMetric')
      }
  }
}
...
const displayedOverviewStats = computed(() =>
  overviewStats.value.map((item) => ({
    ...item,
    ...getOverviewStatPresentation(getOverviewStatKey(item.label)),
...
function translateOverviewStatLabel(label) {
  const statKey = getOverviewStatKey(label)
  return statKey ? t(`pages.home.${statKey}`) : label
}
...
.overview-stat-card-users {
  background:
    linear-gradient(180deg, rgba(243, 248, 255, 0.98), rgba(233, 242, 255, 0.94));
}

.overview-stat-card-coaches {
  background:
    linear-gradient(180deg, rgba(242, 250, 255, 0.98), rgba(232, 245, 255, 0.94));
}

.overview-stat-card-groups {
  background:
    linear-gradient(180deg, rgba(244, 245, 255, 0.98), rgba(236, 239, 255, 0.94));
}

.overview-stat-card-payments {
  background:
    linear-gradient(180deg, rgba(241, 247, 255, 0.98), rgba(231, 239, 250, 0.94));
}

.overview-stat-card-default {
  background:
    linear-gradient(180deg, rgba(255, 255, 255, 0.95), rgba(246, 250, 255, 0.92));
}
...
.admin-home-shell-dark .overview-stat-card {
  ...
  background:
    linear-gradient(180deg, rgba(18, 28, 46, 0.96), rgba(15, 24, 40, 0.94));
  box-shadow:
    0 16px 34px rgba(4, 10, 24, 0.22),
    inset 0 1px 0 rgba(255, 255, 255, 0.06);
}

.admin-home-shell-dark .overview-stat-card-users {
  background:
    linear-gradient(180deg, rgba(20, 33, 56, 0.98), rgba(17, 31, 55, 0.95));
}

.admin-home-shell-dark .overview-stat-card-coaches {
  background:
    linear-gradient(180deg, rgba(18, 36, 58, 0.98), rgba(15, 32, 53, 0.95));
}

.admin-home-shell-dark .overview-stat-card-groups {
  background:
    linear-gradient(180deg, rgba(24, 29, 59, 0.98), rgba(20, 26, 54, 0.95));
}

.admin-home-shell-dark .overview-stat-card-payments {
  background:
    linear-gradient(180deg, rgba(18, 30, 52, 0.98), rgba(15, 27, 48, 0.95));
}
...
.admin-home-shell-dark .summary-side {
...
.admin-home-shell-dark .summary-caption {
  color: #a1b3cf;
}
